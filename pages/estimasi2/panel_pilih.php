                ...
                <div class="table-header">
                    ...
                    <div class="table-info">
                        <span class="info-badge">
                            ...
                            Klik "Pilih" untuk menambahkan panel
                        </span>
                    </div>
                </div>

                <div class="modern-table-container">
                    <table id="panelestimasi" class="modern-table table table-hover">
                        <thead>
                            <tr>
                        ...
                        <tbody>
                            ...
                            echo '<script>console.log("DEBUG: Checking database connection for panel_pilih.php");</script>';
                            ...
                            if (!$rescatat) {
                                ...
                                echo '<tr><td colspan="5" class="text-center">Error loading panel data. Please check database connection.</td></tr>';
                            } else {
                                $panelCount = mysqli_num_rows($rescatat);
                                echo '<script>console.log("DEBUG: Found ' . $panelCount . ' panels in database");</script>';

                                if ($panelCount == 0) {
                                    echo '<tr><td colspan="5" class="text-center">No panels found in database.</td></tr>';
                                } else {
                                    while ($catat = mysqli_fetch_array($rescatat)) {
                                        $diskon = ($catat['diskon'] / 100) * $catat['harga_jual'];
                                        $hartot = $catat['harga_jual'] - $diskon;
                                        echo '<script>console.log("DEBUG: Loading panel: ' . addslashes($catat['nama']) . '");</script>';
                            ?>
                            <tr class="table-row">
                                <td class="table-cell">
                                ...
                                <td class="table-cell text-center">
                                    <button type="button"
                                            ...
                                            onclick="pilihpanele('<?php echo $catat['id_panel']; ?>','<?php echo addslashes($catat['nama']); ?>','<?php echo $catat['harga_jual']; ?>','<?php echo $hartot; ?>','<?php echo $catat['diskon']; ?>');">
...
function pilihpanele(a, b, c, d, e) {
    console.log('DEBUG: pilihpanele called with params:', a, b, c, d, e);
    console.log('DEBUG: ModalPilihPanel element:', $('#ModalPilihPanel'));
    console.log('DEBUG: ModalPilihPanel length:', $('#ModalPilihPanel').length);

    // Check if target elements exist before setting values
    console.log('DEBUG: #panel element exists:', $('#panel').length);
    console.log('DEBUG: #panelnm element exists:', $('#panelnm').length);
    console.log('DEBUG: #hargapokok element exists:', $('#hargapokok').length);
    console.log('DEBUG: #hargatotal element exists:', $('#hargatotal').length);
    console.log('DEBUG: #diskon element exists:', $('#diskon').length);
    ...
    try {
        ...
        console.log('DEBUG: Attempting to hide ModalPilihPanel');
        $("#ModalPilihPanel").modal('hide');

        console.log('DEBUG: ModalPilihPanel hide called successfully');

    } catch (error) {
        console.error('DEBUG: Error in pilihpanele function:', error);
    }
}

// Debug: Check if modal exists on page load
$(document).ready(function() {
    console.log('DEBUG: panel_pilih.php loaded');
    console.log('DEBUG: ModalPilihPanel exists:', $('#ModalPilihPanel').length > 0);
    console.log('DEBUG: ModalPilihPanel content:', $('#ModalPilihPanel').html());
});
</script>
